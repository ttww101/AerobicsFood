...
@objc class AerobicsFoodVC: UIViewController, UITableViewDataSource, UITableViewDelegate {
    ...
    override func viewDidLoad() {
        ...
        self.aerobicsTime = AerobicsFoodData.aerobicsTime
        
        let nib = UINib(nibName: "AerobicsFoodCell", bundle: nil)
    ...
    func getData() {
        listData = [AerobicsFoodMode]()
        let actions = AerobicsFoodData.actions
        let actionName = AerobicsFoodData.actionName
        let actionList: [String] = AerobicsFoodData.actionList["Aerobics"]!
        let actionTime: [Int] = AerobicsFoodData.actionTime["Aerobics"]!
        for i in 0...actionList.count - 1 {
            let imgsCount = actions[actionList[i]]!
            let imgs = getImgsName(actionList[i], imgsCount)
            listData.append(AerobicsFoodMode.init(title: actionName[actionList[i]]!, time: actionTime[i], imgs: imgs))
    ...
    @objc func runAerobics(_ timer: Timer) -> Void {
        ...
        if (recordTime - (Int(recordTime / 40) * 40)) == 0 {
            if row != 0 {
                let subCell = aerobicsTabel.cellForRow(at: IndexPath(row: row - 1, section: 0)) as! AerobicsFoodCell
                subCell.stopImg()
            }
            if aerobicsTime == 0 {
                resetView()
            ...
            let cell = aerobicsTabel.cellForRow(at: indexPath) as! AerobicsFoodCell
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = aerobicsTabel.dequeueReusableCell(withIdentifier: "Cell", for: indexPath) as! AerobicsFoodCell
